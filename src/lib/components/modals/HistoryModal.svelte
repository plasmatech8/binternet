...
<div class="card px-10 py-6 w-modal-wide">
	<div class="flex flex-col gap-6">
		<header class="flex justify-between items-center">
			<div class="text-3xl">
				Order History ({$orderHistoryStore.length})
			</div>
			<div class="text-base opacity-50 ml-2">(on this device)</div>
		</header>

		<div class="overflow-y-auto max-h-[calc(100vh-14rem)] p-1">
			<div class="flex gap-6 flex-wrap">
				{#each $orderHistoryStore as orderStatus}
					<div class="table-container outline outline-1 outline-primary-500">
						<a href={PUBLIC_ORDER_STATUS_URL + orderStatus.id} target="_blank">
							<table class="table">
								<tbody>
									<tr>
